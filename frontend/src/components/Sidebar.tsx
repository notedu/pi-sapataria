...
function ItemNav({ item, aoNavegar }: { item: ItemMenu; aoNavegar: () => void }) {
  ...
  if (item.acao === 'sair') return <div>
    <button disabled={saindo} type="button" className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold text-[#444652] hover:bg-[#d7dae4]/70" onClick={async () => {
      ...
      catch { setErro(t('logoutError')); }
      finally { setSaindo(false); }
    }}><Icone aria-hidden="true" className="size-5" />{saindo ? t('loggingOut') : t('logout')}</button>
...
export default function Sidebar({ itens, itensRodape, aberto, aoFechar }: SidebarProps) {
  ...
  return (
    <>
      {aberto && (
        <button aria-label={t('closeMenu')} className="fixed inset-0 z-40 bg-[#181c23]/35 md:hidden" onClick={aoFechar} type="button" />
      )}
      <aside aria-label={t('mainMenu')} className={`fixed inset-y-0 left-0 z-50 flex w-64 -translate-x-full flex-col border-r border-[#c4c6d4]/40 bg-[#e6e2db] px-2 py-4 transition-transform duration-200 md:translate-x-0 ${aberto ? 'translate-x-0 shadow-xl' : ''}`}>
        <div className="mb-8 flex items-center gap-3 px-2">
          <img alt={t('brand')} className="size-9 rounded-full object-cover" src={logoUrl} />
          <div className="min-w-0 flex-1">
            <h1 className="truncate text-xl font-bold text-[#002c7c]">{t('brand')}</h1>
            <p className="text-xs font-medium text-[#444652]">{t('tagline')}</p>
          </div>
          <button aria-label={t('closeMenu')} className="rounded-md p-1.5 text-[#444652] hover:bg-[#d7dae4] md:hidden" onClick={aoFechar} type="button"><X aria-hidden="true" className="size-5" /></button>
        </div>
        <nav aria-label={t('systemPages')} className="space-y-1">
        ...
        <div className="mt-auto">
          <div className="mb-3 flex items-center gap-3 border-b border-[#c4c6d4]/45 px-2 pb-3">
            ...
            <div><p className="text-sm font-semibold text-[#181c23]">{usuario?.nome}</p><p className="text-xs text-[#444652]">{usuario?.perfil === 'admin' ? t('administrator') : t('employee')}</p></div>
          </div>
          <nav aria-label={t('accountOptions')} className="space-y-1">
